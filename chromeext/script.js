	...
	function cWin(string, type){
		if (localStorage["show_as"]=="tab"){
			
			chrome.tabs.create({
				url: "http://qr-tools.appspot.com/qr" + (localStorage["shorten_url"]=="true" ? "" : "l") + "?"+type+"=" + string
			})
		}
		else{
		...
		/*else if(localStorage["show_as"]=="over"){
			...
			if (div == null){
				...
				s = "http://qr-tools.appspot.com/qr" + (localStorage["shorten_url"] ? "" : "l") + "?"+type+"=" + string
				alert(s)
				qr(s)
			}
		}*/
	...
	function qrt(info){
		...
		cWin(searchstring, 'text');
	}
	
	function qri(info){
		var URL = encodeURI(info.srcUrl);
		cWin(URL, 'url');
	}
	
	function qru(info){
		var URL = encodeURI(info.linkUrl);
		cWin(URL, 'url');
	}
	
	
	
	chrome.contextMenus.create({title: "QR Selected Text", contexts:["selection"], onclick: qrt});
	chrome.contextMenus.create({title: "QR Selected Link URL", contexts:["link"], onclick: qru});
	chrome.contextMenus.create({title: "QR Selected Image URL", contexts:["image"], onclick: qri});
